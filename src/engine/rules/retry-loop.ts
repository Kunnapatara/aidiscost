...
export function evaluateRetryErrorLoop(
  ...
  const annualized = Number((wastedSpend * 30 * 12).toFixed(2));
  ...
  return {
    ...
    summary: `Identified ${loopEvents.length} unbacked retry attempts across ${totalLoopBursts} bursts exhibiting rapid consecutive 429/500 errors without jittered exponential backoff.`,
    ...
    calculation_method: 'Summation of billed tokens on failed retry attempts prior to client circuit breaker or terminal response.',
    assumptions: [
    ...
    evidence: {
      ...
      metrics_comparison: [
        ...
        },
        {
          ...
          current_value: '< 4.0s average interval',
          target_value: 'Exponential Backoff (1s, 2s, 4s, 8s)',
      ...
      mathematical_proof: `Formula: Sum[ Cost_failed_retry_i ] where status in ('ERROR', 'RATE_LIMITED') and retry_interval < 10s = $${wastedSpend.toFixed(4)} recoverable spend across ${loopEvents.length} aborted attempts.`,
    },
    status: 'DETECTED',
